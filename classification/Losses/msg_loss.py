...
class MSGLoss():
    def __init__(self,scales=4,taps=[1,1,1,1], k_size=[3,3,3,3], device=None, eps = 0.00001, mode = 'l2'):
        self.n_scale = scales
        self.taps = taps
        self.k_size = k_size
        self.device = device
    ...
    def forward(self, output, target, mask):
        ...
        for i in range(self.n_scale):
            ...
            mask_resized = torch.floor(self.resize_aa(mask, i)+0.001).to(self.device)
            # erosion to mask out pixels that are effected by unkowns
            mask_eroded = kn_morph.erosion(mask_resized, self.erod_kernels[i].to(self.device))
            
            diff_resized = self.resize_aa(diff, i).to(self.device)
            
            # imshow(mask_resized, 'mask_resized')
            # imshow(diff_resized, 'diff_resized')
            # compute grads
            grad_mag = self.gradient_mag(diff_resized, i, self.mode)
    ...
    def gradient_mag(self, diff, scale, mode='l2'):
        # B*C*H*W
        ...
        if mode == 'l2':
            ...
            grad_magnitude = torch.abs(grad_x - grad_y) + self.eps
