...
from rnd.utils.dataset_multi_channel import NumpyImageFolder
...
def masked_bce_loss(y_pred, y_true, mask):
    ...
    # Apply the mask to ignore missing labels
    masked_bce = bce * mask

    # Normalize by number of known labels (avoid dividing by zero)
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for inputs, labels, _ in train_loader:
        ...
        batch_size, num_classes = labels_onehot.shape
        mask = torch.ones_like(labels_onehot)

        # Find the index of the positive class for each sample
        class_indices = labels_onehot.argmax(dim=1)  # shape: [batch_size]

        for i in range(batch_size):
            idx = class_indices[i].item()
            for offset in [-2, -1, 1, 2]: #neibourhood offsets
                circular_idx = (idx + offset) % num_classes
                mask[i, circular_idx] = 0
        loss = criterion(raw_outputs, labels_onehot.float(), mask=mask)
...
# Use same random split for both datasets
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            os.makedirs(args.save_dir, exist_ok=True)
            torch.save(
                model.state_dict(), os.path.join(args.save_dir, f"{args.model}_final_{epoch}.pth")
            )
    else:

        # Load model if not training
        model.load_state_dict(torch.load(os.path.join(args.save_dir, f"{args.model}_final_40.pth")))
        print(f"Model loaded from {args.save_dir}/{args.model}_final_30.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", type=str, default="./data/fhr4/final_data_21_classifier_2_channels_std")
    parser.add_argument("--train", action="store_true", default=False, help="Set to False to load model and generate GradCAM")
    parser.add_argument("--batch-size", type=int, default=12)
    parser.add_argument("--num-classes", type=int, default=21, help="Number of classes for one-hot encoding")
