"""
Mix Vision Transformer (MiT) backbone, ported from PerspectiveFields.

This is the SegFormer encoder that extracts multi-scale features at
[1/4, 1/8, 1/16, 1/32] resolution. Adapted for configurable input channels
(default 1 for grayscale ILI data).

Variants:
    - mit_b0: lightweight (embed_dims=[32, 64, 160, 256], depths=[2,2,2,2])
    - mit_b3: full (embed_dims=[64, 128, 320, 512], depths=[3,4,18,3])

Reference: PerspectiveFields / SegFormer (NeurIPS 2021).
"""
...
class OverlapPatchEmbed(nn.Module):
    """Overlapping patch embedding for each stage."""
...
class MixVisionTransformer(nn.Module):
    """
    4-stage hierarchical transformer that produces multi-scale features.

    Output: list of 4 tensors at [1/4, 1/8, 1/16, 1/32] resolution with
    channel dimensions ``embed_dims``.
    """
    ...
    def __init__(self, img_size=224, in_chans=3,
        ...
        self.patch_embed4 = OverlapPatchEmbed(img_size=img_size // 16, patch_size=3,
                                              stride=2, in_chans=embed_dims[2],
...
def mit_b0(in_chans=1, **kwargs):
    """Lightweight variant — good for smaller datasets / faster iteration."""
...
def mit_b3(in_chans=1, **kwargs):
    """Full-power variant matching PerspectiveFields."""
    return MixVisionTransformer(
        in_chans=in_chans,
